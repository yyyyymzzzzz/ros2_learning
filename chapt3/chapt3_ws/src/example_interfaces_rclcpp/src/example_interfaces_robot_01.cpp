...
#include "example_interfaces/srv/move_robot.hpp"
#include "example_interfaces/msg/robot_status.hpp"
...
public:
   ...
   * @return float
   */
   float move_distance(float distrance) {
    status_ = example_ros2_interfaces::msg::RobotStatus::STATUS_MOVING;
    target_pose_ += distrance;
    while (std::fabs(target_pose_ - current_pose_) > 0.01) {
      float step = distrance/ std::fabs(distances) * std::fabs(target_pose_ - current_pose_) * 0.1;
      current_pose_ += step;
...
public:
  ExampleInterfacesRobot(std::string name) : Node(name) {
    ...
    robot_status_server_ = this->create_service<example_ros2_interfaces::srv::MoveRobot>(
      "move_robot", std::bind(&ExampleInterfacesRobot::move_robot_callback, this, std::placeholders::_1, std::placeholders::_2));
...
private:
  ...
  rclcpp::Service<example_ros2_interfaces::msg::RobotStatus>::SharedPtr robot_status_publisher_;
